...
% imshow(A);
for ji=1:1:10
%     J2 = impulsenoise(Agr1,0.3,1);
%     J2 = imnoise(Agr1,'gaussian',0,900/255^2);
    J2 = imnoise(Agr1,'salt & pepper',0.2);
    A = im2double(J2);
          flg = 0; 
          for rk=1:1:10
...
% %                             
                            aAvg = median(Med);
                            ftrN(6,i,j) =  (1-Ap)*abs(A(i,j)-aAvg);
                            ftrNF(6,i,j) = Ap+(1-Ap)*betag*(1-ftrN(6,i,j));
                            ftrF(6,i,j) = (1-Ap)*(1-betag)*(1-ftrN(6,i,j));
% %                             
%                             Medj3 = Med;
                            Med(10) = A(i,j);
                            Med(11) = A(i,j);
...
%                             ftrNF(8,i,j) = Ar+(1-Ar)*betar*(1-ftrN(8,i,j));
%                             ftrF(8,i,j) = (1-Ar)*(1-betar)*(1-ftrN(8,i,j));
